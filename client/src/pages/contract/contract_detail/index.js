  ...
  render() {
    ...
    return (
      <div>
        ...
        <Card>
          <p>本合同由作为委托方的<FormattedMessage id={this.props.dataCheck.check.client || ' '}/>(以下简称“甲方”)与作为受托方的南京大学计算机软件新技术国家重点实验室（以下简称“乙方”）在平等自愿的基础上，依据《中华人民共和国合同法》有关规定就项目的执行，经友好协商后订立</p>
          <Divider />
          <h2>一、任务表述</h2>
          <p>乙方按照国家软件测试标准和测试规范，完成甲方委托的软件 <FormattedMessage id={this.props.dataCheck.check.softwareName || ' '}/>（下称受测软件）的质量特性 <FormattedMessage id={this.props.dataCheck.check.quality || ' '}/> 进行测试，并出具相应的测试报告</p>
          <Divider />
          <h2>二、双方的主要义务</h2>
          <div>1.甲方的主要义务</div>
          ...
          <Divider />
          <h2>四、合同价款</h2>
          <p>本合同软件测试费用为人民币<FormattedMessage id={this.props.dataCheck.check.price || ' '}/>(¥<FormattedMessage id={this.props.dataCheck.check.price || ' '}/>元)。</p>
          <Divider />
          <h2>五、测试费用支付方式</h2>
          <div>本合同签订后，十个工作日内甲方合同价款至乙方账户</div>
          <Divider />
          <h2>六、履行的期限</h2>
          <div>1.本次测试的履行期限为合同生效之日起 <FormattedMessage id={this.props.dataCheck.check.finishTime || ' '}/> 个自然日内完成。</div>
          <div>2.经甲乙双方同意，可对测试进度作适当修改，并以修改后的测试进度作为本合同执行的期限。</div>
          <div>3.如受测软件在测试过程中出现的问题，导致继续进行测试会影响整体测试进度，则乙方暂停测试并以书面形式通知甲方进行整改。在整个测试过程中，整改次数限于<FormattedMessage id={this.props.dataCheck.check.modifyNum || ' '}/>次，每次不超过<FormattedMessage id={this.props.dataCheck.check.modifyTime || ' '}/>天。</div>
